    ...
    */
    ...
    func getCalendarTime() -> Void {
        ...
        APIManager.sharedInstance.getDataToURL(url: CALENDAR_TIME, parameters: dictParam, onCompletion: {(response) in
            ...
            if (response.result.error != nil) {
                ProjectCommon.initAlertView(viewController: self, title: "", message: "Không thể lấy thông tin về lịch làm việc lúc này,vui lòng quay lại sau", buttonArray: ["Đóng"], onCompletion: { (index) in
    ...
    func postCalendarTime(object:CalendarTimeObject) -> Void {
        ...
        APIManager.sharedInstance.postDataToURL(url: CALENDAR_TIME, parameters: dictParam, onCompletion: {(response) in
            ...
            Lib.removeLoadingView(on: self.view)
            if (response.result.error != nil) {
                ProjectCommon.initAlertView(viewController: self, title: "", message: "Không thể cập nhật thông tin về lịch làm việc lúc này,vui lòng quay lại sau", buttonArray: ["Đóng"], onCompletion: { (index) in
    ...
    func deleteCalendarTime(object:CalendarTimeObject) -> Void {
        ...
        APIManager.sharedInstance.deleteDataToURL(url: CALENDAR_TIME, parameters: dictParam, onCompletion: {(response) in
            print(response)
             Lib.removeLoadingView(on: self.view)
            if (response.result.error != nil) {
                ProjectCommon.initAlertView(viewController: self, title: "", message: "Không thể xoá thông tin về lịch làm việc lúc này,vui lòng quay lại sau", buttonArray: ["Đóng"], onCompletion: { (index) in
                })
            }else {
                let resultDictionary = response.result.value as! [String:AnyObject]
                if (resultDictionary["status"] as! NSNumber) == 1 {
