...
#define BACKGROUND_COLOR [UIColor colorWithRed:100/255.0 green:155/255.0 blue:245/255.0 alpha:1]
...
- (instancetype)initWithSectionImages:(NSArray *)sectionImages sectionSelectedImages:(NSArray *)sectionSelectedImages titlesForSections:(NSArray *)sectiontitles lockForSection:(NSArray *)sectionLockImages {
    self = [super initWithFrame:CGRectZero];
    ...
    if (self) {
        ...
        self.sectionSelectedImages = sectionSelectedImages;
        self.sectionTitles = sectiontitles;
        self.sectionLockImages = sectionLockImages;
        self.type = HMSegmentedControlTypeTextImagesAndLock;
    }
...
- (void)setSectionLockImages:(NSArray *)sectionLockImages {
    _sectionLockImages = sectionLockImages;
...
//			CGFloat yOffset = roundf(((CGRectGetHeight(self.frame) - self.selectionIndicatorHeight) / 2) - (stringHeight / 2));
            CGFloat stringHeight = 16;
            CGFloat yOffset = roundf(((CGRectGetHeight(self.frame) - self.selectionIndicatorHeight) / 2) - (stringHeight));
...
//            CGFloat imageYOffset = roundf((CGRectGetHeight(self.frame) - self.selectionIndicatorHeight) / 2.0f);
            CGFloat imageYOffset = roundf((CGRectGetHeight(self.frame) - self.selectionIndicatorHeight) / 2.0f - imageHeight/2 + 1);
...
//            CGRect textRect = CGRectMake(textXOffset, yOffset, textWidth, stringHeight);
            CGRect textRect = CGRectMake(imageXOffset + imageWidth - 15, yOffset, textWidth, stringHeight);
            ...
            titleLayer.alignmentMode = kCAAlignmentCenter;
            NSAttributedString * strTitles = [self attributedTitleAtIndex:idx];
            if ([strTitles.string isEqualToString:@""] || [strTitles.string isEqualToString:@"0"]) {
               
            }
            else{
                titleLayer.backgroundColor = [UIColor colorWithRed:248/255.0f green:135/255.0f blue:10/255.0f alpha:1].CGColor;
                titleLayer.cornerRadius = stringHeight/2;
                titleLayer.string = strTitles;
            }
    ...
    else if (self.type == HMSegmentedControlTypeTextImagesAndLock){
        [self.sectionImages enumerateObjectsUsingBlock:^(id iconImage, NSUInteger idx, BOOL *stop) {
            UIImage *icon = iconImage;
            //            CGFloat imageWidth = icon.size.width;
            //            CGFloat imageHeight = icon.size.height;
            ...
            CGFloat stringHeight = [self measureTitleAtIndex:idx].height;
            //			CGFloat yOffset = roundf(((CGRectGetHeight(self.frame) - self.selectionIndicatorHeight) / 2) - (stringHeight / 2));
            CGFloat yOffset = roundf(((CGRectGetHeight(self.frame) - self.selectionIndicatorHeight) / 2) + imageHeight /2 - 5);
            ...
            //            CGFloat imageYOffset = roundf((CGRectGetHeight(self.frame) - self.selectionIndicatorHeight) / 2.0f);
            CGFloat imageYOffset = roundf((CGRectGetHeight(self.frame) - self.selectionIndicatorHeight) / 2.0f - yOffset + 10);
            CGRect imageRect = CGRectMake(imageXOffset, imageYOffset, imageWidth, imageHeight);
            CGRect textRect = CGRectMake(textXOffset, yOffset, textWidth, stringHeight);
            ...
            CALayer * imgLockLayer = [CALayer layer];
            imgLockLayer.frame = CGRectMake(imageRect.origin.x + imageRect.size.width - 10, imageRect.origin.y - 2, 15, 15);
            UIImage * imgLock = [self.sectionLockImages objectAtIndex:idx];
            imgLockLayer.contents = (id)imgLock.CGImage;
            ...
            [self.scrollView.layer addSublayer:imgLockLayer];
...
//            self.segmentWidth = MAX(stringWidth, self.segmentWidth);
    ...
    else if (self.type == HMSegmentedControlTypeTextImagesAndLock && self.segmentWidthStyle == HMSegmentedControlSegmentWidthStyleFixed){
        //lets just use the title.. we will assume it is wider then images...
        [self.sectionTitles enumerateObjectsUsingBlock:^(id titleString, NSUInteger idx, BOOL *stop) {
            //            CGFloat stringWidth = [self measureTitleAtIndex:idx].width + self.segmentEdgeInset.left + self.segmentEdgeInset.right;
            //            self.segmentWidth = MAX(stringWidth, self.segmentWidth);
...
- (NSUInteger)sectionCount {
    if (self.type == HMSegmentedControlTypeText) {
        return self.sectionTitles.count;
               ...
               self.type == HMSegmentedControlTypeTextImages||self.type == HMSegmentedControlTypeTextImagesAndLock) {
...
- (void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event {
    ...
    if (CGRectContainsPoint(self.bounds, touchLocation)) {
        ...
        } else if (self.type == HMSegmentedControlTypeTextImages || self.type == HMSegmentedControlTypeText || self.type == HMSegmentedControlTypeTextImagesAndLock) {
...
- (NSDictionary *)resultingTitleTextAttributes {
    ...
    if (self.type == HMSegmentedControlTypeTextImages) {
        defaults = @{
